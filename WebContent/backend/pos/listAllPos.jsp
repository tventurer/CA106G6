...
<head>
<meta charset="UTF-8">
<title>所有文章資料 - listAllPos.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>所有文章資料 - listAllPos.jsp</h3>
		 <h4><a href="<%=request.getContextPath()%>/backend/pos/select_page.jsp">
...
<table>
	...
	<c:forEach var="posVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${posVO.postime}</td> 
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/backend/pos/pos" style="margin-bottom: 0px;">
